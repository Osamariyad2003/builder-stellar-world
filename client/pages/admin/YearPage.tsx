    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Manage subjects and lectures for this year</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            onClick={async () => {
              const url = window.prompt("Enter image URL for this year:", year.imageUrl || "");
              if (url === null) return;
              try {
          ...
          >
            <Upload className="h-4 w-4 mr-2" /> {year.imageUrl ? "Change Image" : "Add Image"}
          </Button>
          {year.imageUrl && (
            <Button
              variant="ghost"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            Subjects
            <Badge variant="secondary">{year.subjects?.length || 0} subjects</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex justify-end mb-4">
            <Button onClick={() => setIsSubjectFormOpen(true)} className="flex items-center gap-2">
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {subject.imageUrl ? (
                          <img src={subject.imageUrl} alt={subject.name} className="w-8 h-8 object-cover rounded" />
                        ) : (
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{(subject.lectures || []).length} lectures</div>
                        </div>
                      </div>

                      <div className="flex gap-2">
                        <Button onClick={() => { setSelectedSubject(subject); setIsLectureFormOpen(true); }} variant="ghost" size="sm">Add Lecture</Button>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-destructive"
                            ...
                            <CardContent className="p-3">
                              <div className="flex items-start gap-3">
                                <div className="relative w-20 h-14 rounded-md overflow-hidden bg-muted flex-shrink-0">
                                  {lecture.imageUrl ? (
                                    <img src={lecture.imageUrl} alt={lecture.name} className="w-full h-full object-cover" />
                                  ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{lecture.description || "No description available"}</p>
                                </div>
                                <div className="flex items-center gap-2">
      ...
      {isLectureFormOpen && selectedSubject && (
        <LectureForm
          ...
          yearType={year.type}
          onClose={() => { setIsLectureFormOpen(false); setSelectedSubject(null); }}
